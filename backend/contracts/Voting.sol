...
contract Voting {
    ...
    struct Vote{
        ...
        address owner;
        uint256 endTime;
        uint256[] votes; // stores the votes for each voter
        mapping(address => bool) voted; // user already voted
        uint256 options;
    ...
    modifier isMember(){
        require(members[msg.sender], "Not a member. Register to become a member");
        _;
    }

    modifier canVote(uint voteId, uint option){
        require(voteId < nextVoteId, "vote doesn't exist");
        require(option < votes[voteId].options, "Invalid option");
        require(!votes[voteId].voted[msg.sender], "Vote already casted");
    ...
    function createVote(string memory uri, uint endTime, uint options) external isMember {
        require(options >=2 && options <= 8, "number of options must be between 1 and 3");
    ...
    function vote(uint256 voteId, uint256 option) 
        external 
        isMember canVote(voteId, option){
